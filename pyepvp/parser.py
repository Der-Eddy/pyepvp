...
def getUpdates(session, url):
    ...
    session._notifications['unread_private_messages'] = int(match("<a href=\"private\.php\"\D+\">(\d+)<\/a>", content, 0))
    session._notifications['unread_vistor_messages'] = int(match("Unread Visitor Messages\D+\n.*tab=visitor_messaging#visitor_messagin\D+\">(\d+)<\/a>", content, 0))
    session._notifications['unapproved_vistor_messages'] = int(match("Unapproved Visitor Messages\D+\n.*tab=visitor_messaging#visitor_messagin\D+\">(\d+)<\/a>", content, 0))
    session._notifications['incoming_friend_requests'] = int(match("profile\.php\?do=buddylist#irc\">(\d+)<\/a>", content, 0))
    session._notifications['groups_request'] = int(match("href=\"group\.php\?do=requests\">(\d+)<\/a>", content, 0))
    session._notifications['groups_invitations'] = int(match("href=\"group\.php\?do=invitations\">(\d+)<\/a>", content, 0))
    session._notifications['unread_picture_comments'] = int(match("href=\"album\.php\?do=unread\">(\d+)<\/a>", content, 0))
    session._notifications['unapproved_picture_comments'] = int(match("href=\"album\.php\?do=moderated\">(\d+)<\/a>", content, 0))
    session._notifications['unapproved_group_messages'] = int(match("href=\"group\.php\?do=moderatedgms\">(\d+)<\/a>", content, 0))
    session._notifications['new_mentions'] = int(match("href=\"usertag\.php\?do=profilenotif&amp;tab=mentions\">(\d+)<\/a>", content, 0))
    session._notifications['new_post_quotes'] = int(match("href=\"usertag\.php\?do=profilenotif&amp;tab=quotes\">(\d+)<\/a>", content, 0))
    session._notifications['staff_changes'] = int(match("do=staffchanges\">(\d+)<\/a>", content, 0))
    session._notifications['subscribed_threads'] = int(match("New Subscribed Threads<span class=\"normal\">: \((\d+)\)<\/span>", content, 0))
    session.elite_gold = int(match("<span class=\"gold\">(\d+)<\/span", content, 0))
